...
class MyConfig(Config):
    def __init__(self,LEARNING_RATE, LR_STEPS, BACKBONE,  NUM_CLASSES , class_names, IMAGES_PER_GPU, IMAGE_MAX_DIM, IMAGE_MIN_DIM, DETECTION_MIN_CONFIDENCE, IMAGE_RESIZE_MODE,GPU_COUNT):
        super().__init__(BACKBONE,  NUM_CLASSES , class_names, IMAGES_PER_GPU, IMAGE_MAX_DIM, IMAGE_MIN_DIM, DETECTION_MIN_CONFIDENCE, IMAGE_RESIZE_MODE,GPU_COUNT)
        ...
        #self.TRAIN_BN = None
        #self.DETECTION_MAX_INSTANCES = 10
...
class instance_custom_training:
    ...
    def modelConfig(self,network_backbone = "resnet101",  num_classes =  1,  class_names = ["BG"], batch_size = 1, detection_threshold = 0.7, image_max_dim = 512, image_min_dim = 512, image_resize_mode ="square", gpu_count = 1, learning_rate=0.001, lr_steps=10):
        self.config = MyConfig(LEARNING_RATE = learning_rate, LR_STEPS=lr_steps, BACKBONE = network_backbone, NUM_CLASSES = 1 +  num_classes,  class_names = class_names, 
    ...
    def load_dataset(self, dataset):
        ...
        labelme_folder2 = os.path.abspath(os.path.join(dataset, "train"))

        #dir where the converted json files will be saved
        save_json_path2 = os.path.abspath(os.path.join(dataset, "train.json"))
